...
function MenuItem({
  icon, label, subtitle, onPress, danger, external,
}: {
  icon: string;
  label: string;
  subtitle?: string;
      ...
      {isAuthenticated ? (
        <>
          ...
          <View style={[styles.section, { backgroundColor: C.surface }]}>
            ...
            {(role === "driver" || role === "both") && (
              <MenuItem
                ...
                subtitle={profile?.dotNumber ? `DOT #${profile.dotNumber}` : "Add DOT & insurance info"}
                onPress={() => router.push("/profile-setup")}
              />
            )}
            {(role === "driver" || role === "both") && (
              <MenuItem
                icon="navigation"
                label="Post My Route"
                subtitle="Let shippers find you on backhaul board"
                onPress={() => {}}
              />
            )}
            <ThemeToggle />
          ...
          <View
            style={[
              styles.section,
              { backgroundColor: "#FEF2F2", borderWidth: 1, borderColor: "#FECACA", marginTop: 12 },
            ]}
          >
            ...
            <View style={deleteStyles.btnRow}>
              ...
              <Pressable
                style={[
                  ...
                  {
                    backgroundColor:
                      deleteConfirmText === "DELETE" && !deleteLoading ? "#DC2626" : "#FCA5A5",
                  },
                ]}
                onPress={handleDeleteAccount}
                disabled={deleteConfirmText !== "DELETE" || deleteLoading}
              >
                <Text style={[deleteStyles.btnText, { color: "#fff" }]}>
...
const styles = StyleSheet.create({
  ...
  profileCard: {
    ...
    shadowColor: "#1A56DB", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.25,
  ...
  sectionTitle: {
    fontFamily: "Inter_600SemiBold", fontSize: 11, paddingHorizontal: 16, paddingTop: 14,
    paddingBottom: 6, letterSpacing: 0.5,
  },
  menuItem: {
